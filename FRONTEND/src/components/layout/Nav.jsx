  ...
  return (
    <div className='fixed z-999 inset-x-0 top-0 p-6 mb-8 flex justify-between items-center bg-neutral-100 dark:bg-neutral-950 transition-colors duration-150'>
      ...
      <div className='flex items-center gap-4'>
        ...
        {
          user ? 
          <Link to={pathname!=="/me" ? "/me" : "/settings"} className='flex gap-2 dark:text-[#707070] text-neutral-500 p-2 transition-colors *:transition-colors duration-150 '>
            { pathname!=="/me" ? <HiOutlineUser size={24}/> : <IoSettingsOutline size={24}/>}
          </Link>
          : 
          pathname==="/login" || pathname==="/register" ||pathname=== "/login-with-email"? <></> : <Link to={"/login"} className='p-2 transition-colors duration-150 hover:opacity-95 bg-black text-sm text-white dark:text-black dark:bg-white rounded-xl cursor-pointer'>Log In</Link>
        }
