...
const Dropdown = ({data, defaultVal}) => {
    ...
    <div ref={dropRef} onClick={()=>setOpen(!open)} className='**:transition-colors duration-150 dark:bg-neutral-900 bg-neutral-100 cursor-pointer flex w-30 justify-between items-center p-2 rounded-xl text-base relative'>
        ...
        <AnimatePresence>
        { open &&
        <Motion.div onClick={(e)=> e.stopPropagation()} initial={{opacity:0, y:-10}} animate={{opacity:1, y:0}} exit={{opacity:0, y:-10}} transition={{duration:0.1}} className='absolute z-999 p-2 shadow-lg bg-neutral-100 rounded-xl top-[110%] inset-x-0 flex flex-col gap-2 dark:bg-neutral-900 max-h-60 overflow-y-auto'>
            {
                data.map((d,index)=> (
